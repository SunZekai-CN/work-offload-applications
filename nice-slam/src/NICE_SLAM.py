...
class NICE_SLAM():
    ...
    def tracking(self, rank):
        ...
        while True:
            if self.mapping_first_frame[0] == 1 or self.cfg['role'] == 'client':
                break
            time.sleep(1)
    ...
    def run(self):
        ...
        for rank in range(3):
            if rank == 0:
                p = threading.Thread(target=self.tracking, args=(rank, ))
            elif rank == 1:
                p = threading.Thread(target=self.mapping, args=(rank, ))
            elif rank == 2:
                if self.coarse:
                    p = threading.Thread(target=self.coarse_mapping, args=(rank, ))
                else:
                    continue
            p.start()
            processes.append(p)
