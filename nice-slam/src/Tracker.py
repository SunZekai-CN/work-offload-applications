...
class Tracker(object):
    ...
    def update_para_from_mapping(self):
        ...
        if self.mapping_idx[0] != self.prev_mapping_idx:
            ...
            for key, val in self.shared_c.items():
                val = val.clone().detach().to(self.device)
                self.c[key] = val
    ...
    def run(self):
        ...
        if self.cfg['role'] == 'server':
            ...
            socket.bind(f"tcp://*:{self.cfg['offload_addr'].split(':')[1]}")
        elif self.cfg['role'] == 'client':
            context = zmq.Context()
            socket = context.socket(zmq.REQ)
            socket.connect(f"tcp://{self.cfg['offload_addr']}")
            time.sleep(1)
        elif self.cfg['role'] == 'self':
            pass
        else:
            raise RuntimeError(f'Unknown role {self.cfg["role"]}')
        ...
        for idx, gt_color, gt_depth, gt_c2w in pbar:
            ...
            if self.cfg['role'] == 'server':
                ...
                _, gt_color, gt_depth, gt_c2w = pickle.loads(socket.recv())
                tbegin = time.time()
                print('server received request ')
            elif self.cfg['role'] == 'client':
                print('client sending request')
                tbegin = time.time()
                socket.send(pickle.dumps((idx, gt_color, gt_depth, gt_c2w)))
                print('client waiting for response')
                res = pickle.loads(socket.recv())
                tend = time.time()
                print('client received response')
                print(f'total {(tend-tbegin):.3f}s')
                continue
            elif self.cfg['role'] == 'self':
                tbegin = time.time()
            ...
            if self.cfg['role'] == 'server':
                ...
                print(f'compute {(tend-tbegin):.3f}s')
            elif self.cfg['role'] == 'client':
                pass
            elif self.cfg['role'] == 'self':
                tend = time.time()
